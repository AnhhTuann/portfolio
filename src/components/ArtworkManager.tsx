...
export default function ArtworkManager() {
  ...
  return (
    <div className="mt-12 border-t border-white/[0.05] pt-12 relative z-10 w-full max-w-2xl mx-auto mb-20">
      <div className="flex justify-between items-center mb-8">
        <div>
          <h2 className="font-display text-3xl text-white font-light tracking-tight mb-2">Artistic Vision.</h2>
      ...
      {loading ? (
        ...
        <div className="columns-2 sm:columns-3 gap-4 space-y-4">
          {artworks.map(a => (
            <div key={a.id} className="relative group break-inside-avoid bg-white/[0.02] border border-white/[0.05] rounded-xl overflow-hidden hover:border-white/[0.1] transition-all">
              ...
              <div className="w-full relative bg-black/50">
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/30 to-transparent opacity-0 group-hover:opacity-100 transition-all duration-300 flex flex-col justify-end p-3 backdrop-blur-[2px]">
                  <h4 className="text-white font-serif font-semibold text-xs tracking-wide truncate mb-0.5">{a.title}</h4>
                  <p className="text-gray-400 text-[9px] font-sans line-clamp-2 mb-3">{a.description}</p>
                  <div className="flex items-center gap-2">
                    <button onClick={() => openEditModal(a)} className="flex-1 py-1.5 bg-[#8B5CF6]/20 text-[#8B5CF6] hover:bg-[#8B5CF6] hover:text-white rounded text-[10px] font-mono transition-colors flex justify-center items-center">
                      SỬA
                    </button>
                    <button onClick={() => handleDelete(a.id)} className="w-8 h-6 flex justify-center items-center bg-red-500/20 text-red-500 hover:bg-red-500 hover:text-white rounded transition-colors">
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-[60] bg-black/95 flex flex-col items-center justify-center p-4 backdrop-blur-xl overflow-y-auto">
          <motion.div 
            ...
            className="relative w-full max-w-xl bg-black border border-white/10 rounded-2xl shadow-[0_0_50px_rgba(139,92,246,0.15)] my-auto"
          >
            <div className="p-4 border-b border-white/10 flex justify-between items-center bg-[#8B5CF6]/5 sticky top-0 z-10 rounded-t-2xl">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-6 max-h-[80vh] overflow-y-auto custom-scrollbar">
              <div className="space-y-2">
                ...
                <input 
                  ...
                  className="w-full bg-white/[0.02] border border-white/[0.05] rounded-xl px-4 py-3 text-white font-serif placeholder:text-gray-600 focus:outline-none focus:border-white/[0.2] transition-colors"
              ...
              <div className="space-y-2">
                ...
                <div className="relative min-h-[200px] w-full border border-dashed border-[#8B5CF6]/30 rounded-xl overflow-hidden bg-[#050505] flex flex-col items-center justify-center group hover:border-[#8B5CF6]/60 transition-colors p-2">
                  {currentArtwork.imageUrl ? (
                    <div className="relative max-h-[400px] w-full flex items-center justify-center bg-black/20 rounded-lg overflow-hidden">
                      ...
                      <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center backdrop-blur-sm">
                        <button type="button" onClick={() => fileInputRef.current?.click()} className="px-4 py-2 border border-white/20 rounded-lg text-white font-mono text-xs hover:bg-white/10 flex items-center gap-2"><Upload className="w-4 h-4"/> ĐỔI CUỘN TRANH</button>
                      </div>
                    </div>
                  ) : (
              ...
              <div className="space-y-2">
                <label className="block font-mono text-[10px] text-gray-400 uppercase tracking-widest">Lời tự sự (Câu chuyện phía sau)</label>
                <textarea 
                  required rows={4} value={currentArtwork.description || ''}
                  onChange={(e) => setCurrentArtwork({...currentArtwork, description: e.target.value})}
                  className="w-full bg-white/[0.02] border border-white/[0.05] rounded-xl px-4 py-3 text-white placeholder:text-gray-600 focus:outline-none focus:border-white/[0.2] transition-colors resize-none text-sm font-light italic"
              ...
              <div className="pt-8">
                <button 
                  ...
                  className="w-full py-4 bg-[#8B5CF6] text-white font-semibold rounded-xl font-mono text-xs tracking-widest hover:bg-[#A78BFA] transition-all flex items-center justify-center gap-2 hover:shadow-[0_0_20px_rgba(139,92,246,0.3)] active:scale-[0.98] disabled:opacity-50"
                >
