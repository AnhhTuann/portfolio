  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
      {skills.map((category, idx) => (
        <div 
          key={idx} 
          className="group p-10 bg-white/[0.02] backdrop-blur-md border border-white/[0.05] rounded-[2rem] hover:border-[#FFC107]/30 hover:bg-white/[0.04] transition-all duration-500"
        >
          <h3 className="font-serif italic text-2xl text-white mb-8 tracking-wide group-hover:text-[#FFC107] transition-colors duration-500 flex items-center gap-3">
            <span className="w-8 h-[1px] bg-[#FFC107]/50 block"></span>
            {category.title}
          </h3>
          <ul className="space-y-5">
            {category.items.map((item, i) => (
              <li key={i} className="text-gray-400 font-sans font-light flex items-center gap-4 group/item">
